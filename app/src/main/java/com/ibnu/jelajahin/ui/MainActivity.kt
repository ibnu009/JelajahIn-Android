...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        navControllerBottomBar.addOnDestinationChangedListener { _, destination, _ ->
            if (destination.id == R.id.homeFragment || destination.id == R.id.eventFragment || destination.id == R.id.bookmarkFragment || destination.id == R.id.discoveryFragment || destination.id == R.id.profileFragment) {
